...
#case 1)
str_replace_na(stud$DEPTNO2, stud$DEPTNO1)          #벡터연산 불가
#NOTE: argument 'replacement' should be a single character string

#case 2)
stud$DEPTNO <- stud$DEPTNO2
std$DEPTNO[is.na(stud$DEPTNO)] <- stud$DEPTNO1      #length error
stud$DEPTNO[is.na(stud$DEPTNO)] <- stud$DEPTNO1[is.na(stud$DEPTNO)]
...
str(prof)         #data frame information
...
#case 2) preferred
prof$BONUS[is.na(prof$BONUS)] <- mean(prof$BONUS, na.rm = T)
...
df2 <- read.csv('data/data2.csv', fileEncoding = 'cp949')
