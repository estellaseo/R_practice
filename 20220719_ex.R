...
#    표준몸무게 = (키 - 100) * 0.9
stud$BMI <- ifelse(stud$WEIGHT > (stud$HEIGHT - 100) * 0.9, '비만', 
       ifelse(stud$WEIGHT == (stud$HEIGHT - 100) * 0.9, '정상', '저체중'))
...
# 2) 결측치가 포함된 날짜는 삭제
con1 <- str_detect(df1$a, '[0-9]') & str_detect(df1$b, '[0-9]')
con2 <- str_detect(df1$c, '[0-9]') & str_detect(df1$d, '[0-9]')
df1 <- df1[con1 & con2, ]


# 3) b지점에서 판매량이 가장 많았던 날짜 확인
max(as.numeric(df1$b), na.rm = T)
